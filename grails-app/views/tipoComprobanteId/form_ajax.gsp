    ...
    <g:form class="form-horizontal" name="frmTipoComprobanteId" role="form" action="save_ajax" method="POST">
        ...
        <div class="form-group ${hasErrors(bean: tipoComprobanteIdInstance, field: 'tipoIdentificacion', 'error')} required">
            <span class="grupo">
                ...
                <div class="col-md-6">
                    <g:select id="tipoIdentificacion" name="tipoIdentificacion.id" from="${cratos.sri.TipoIdentificacion.list()}" optionKey="id" required="" optionValue="descripcion" value="${tipoComprobanteIdInstance?.tipoIdentificacion?.id}" class="many-to-one form-control"/>
                </div>
                *
            </span>
        </div>
